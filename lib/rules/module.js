        ...
        estraverse.traverse(ast, {
            enter: function (node, parent) {
                if (node.type === SYNTAX.CallExpression && node.callee.name === 'define') {
                    ...
                    if (!args.length) {
                        self.reporter.error(file.path, '`define`缺少必要参数', node.loc.start.line, node.loc.start.column);
                        return;
                    }
                    // 具名模块定义
                    if (args[0].type !== SYNTAX.FunctionExpression) {
                        self.reporter.error(file.path, '`define`必须使用匿名模块定义', node.loc.start.line, node.loc.start.column);
                        return;
                    }
